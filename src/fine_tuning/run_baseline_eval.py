...
for i, prompt in enumerate(prompts):
    ...
    # THE FIX: Wrap the prompt in CodeLlama's required instruction tags
    ...
    # Print the raw output to the terminal so we can watch it work
    print(f"\n--- Raw AI Output {i+1} ---")
    ...
    file_path = f"eval_outputs_baseline/baseline_output_{i+1}.py"
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(clean_code)
    
    # 4. Run Bandit 
    print(f"Scanning baseline code with Bandit...")
